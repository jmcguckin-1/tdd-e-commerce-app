...
function NSItem(){
    ...
    // styling methods
     ...
     // Retrieves product inventory
       useEffect(() => {
        fetch("/get_product_inventory/NS/user" + "/" + i18n.language)
    ...
    function displayGrid(){
     ...
     document.getElementsByClassName('itemLength')[0].style.display = 'block';
      document.getElementById('itemLang').style.left = "2%";
        ...
        // checks if user has items in their cart
         useEffect(() => {
         if(currentUser){
  ...
  list: [
    {
        ...
        // checks if the user has a cart already
        if (currentCart){
            cart = updated
            existingCart = true;
        }
    ...
    applyStyles();
     ...
     function sort(){
      document.getElementById('loading_screen').style.display='block';
        ...
        let category = "Nintendo Switch Games";
    ...
    return (
        ...
        <NavBar/>
             ...
             <h2 className='searchRes'>{t("nin")} {t('home')}</h2>
            <h3 className='itemLength'>{productInventory.length} Nintendo Switch {t('found')}</h3>
